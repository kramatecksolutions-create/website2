...
import {BrainCircuit, BriefcaseBusiness, Activity, ArrowRight} from 'lucide-react'

const roles=[
  {icon:BrainCircuit,title:'AI Engineer',summary:'Build intelligent enterprise solutions that connect AI to real business workflows.',items:['Build and integrate AI/LLM solutions, RAG, agents, APIs, and automation workflows.','Work with cloud AI platforms and enterprise applications.'],skills:'Python/Java, APIs, LLMs, RAG, vector databases, AWS/Azure/GCP.'},
  {icon:BriefcaseBusiness,title:'Product Manager',summary:'Shape useful products from customer problem through roadmap, release and feedback.',items:['Define product vision, roadmap, requirements, and priorities.','Work with customers, engineering, design, and business teams.','Own product documentation, feature planning, releases, and feedback.'],skills:'Product strategy, Agile, Jira, stakeholder management; AI/SaaS experience preferred.'},
  {icon:Activity,title:'Operations Engineer',summary:'Keep modern cloud platforms reliable, observable and ready to scale.',items:['Support cloud infrastructure, deployments, monitoring, reliability, and incident management.','Automate operational tasks and improve system performance.'],skills:'AWS/Azure/GCP, Linux, Docker/Kubernetes, CI/CD, monitoring, scripting.'}
]
...
export default function Careers(){
  ...
  return <>
    ...
    <section className="section careers-section"><div className="container">
      ...
      <div className="career-grid">{roles.map((r,i)=>{const Icon=r.icon;return <article className="career-card" key={r.title}><div className="career-top"><div className="career-icon"><Icon/></div><span>0{i+1}</span></div><h3>{r.title}</h3><p className="career-summary">{r.summary}</p><ul>{r.items.map(x=><li key={x}>{x}</li>)}</ul><div className="career-skills"><strong>Skills</strong><p>{r.skills}</p></div><button type="button" className="career-apply" onClick={()=>{setApplyRole(r.title);setStatus('');setTimeout(()=>document.getElementById('career-application')?.scrollIntoView({behavior:'smooth'}),50)}}>Apply for this role <ArrowRight size={18}/></button></article>})}</div>
