...
export default function Insights(){return <><PageHero eyebrow="INSIGHTS" title="Ideas for practical enterprise intelligence." description="Perspectives on AI, data, automation, cloud and digital engineering." image="/assets/visuals/insights.svg"/><section className="section services-section"><div className="container"><div className="insight-grid">{insights.map(x=><a className="insight-card" href={`/insights/${x.slug}`} key={x.slug}><img src={x.image} alt=""/><div><span>{x.category}</span><h3>{x.title}</h3><p>{x.summary}</p><b>Read insight →</b></div></a>)}</div></div></section></>}
